...
def calculate_incremental_quadratic_variation(df, price_column="Close", time_step=1):
    ...
    incremental_qv_series = pd.Series(
        incremental_qv, index=df.index, name="incremental_qv"
    )
...
def create_target(df, Close_col, num_candles):
    ...
    for i in range(n):
        ...
        if (
            Close_prices[i] == max(local_window)
            and local_window.tolist().count(Close_prices[i]) == 1
        ):
            target[i] = 1
        elif (
            Close_prices[i] == min(local_window)
            and local_window.tolist().count(Close_prices[i]) == 1
        ):
            target[i] = 2
